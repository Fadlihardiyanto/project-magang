...
<div class="card mb-3">
    ...
    <div class="card-body" id="hideshow">
        ...
        <div class="col-lg-7 ms-auto me-auto">
            <form method="post" action="{{ url('dashboard/kereta/'.$kereta->id) }}">
            @csrf
            @method('PUT')
            <div class="mb-3 d-flex">
              <label class="p-2 col-2 fw-bold" for="name" class="form-label">Nama Kereta</label>
              <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" required value="{{ old('name', $kereta->name) }}">
              @error('name')    
